...
controller.checkLoginUserWithEmail = ( req , res ) => {
    ...
    JobAppliedCandidateCl.findOne( where )
    .then( (loginData)=>{  
        ...
        if( loginData ){ 
            ...
            JobAppliedCandidateCl.updateOne( { _id: loginData._id }, {$set: saveData} )
            .then( (data)=>{ 
                if( data ){
...
controller.verifyOTP = ( req , res ) => {
    ...
    JobAppliedCandidateCl.findOne( {email: email_id } )
    .then( (loginData)=>{ 
        ...
        if( loginData ){
            ...
            if( loginData.otp.toString() !== otp.toString() ){
                return res.status(200).send( {'status':false, 'message':'OTP not matched'} );
            }

            const timeDifference = calculateTime( loginData.otp_sent_on , dbDateFormat() );

            if( timeDifference > 10 ){
                return res.status(200).send( {'status':false, 'message':'OTP Expired'} );
            } 
