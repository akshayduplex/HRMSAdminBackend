const SalaryRangeCI = require('../../../models/SalaryRangeCI.js')
...
/********* Add New SalaryRange Data **********/
controller.AddSalaryRangeData = ( req, res )=>{
    ...
    SalaryRangeCI.findOne( { label: saveData.label } )
    .then( (ckData)=>{
        if( ckData ){
            return res.status(409).send( {'status':false, 'message': 'Salary Range Already Added'} );
        }
    
        const instData = new SalaryRangeCI( saveData );
        instData.save()
        .then( (data)=>{
            return res.status(200).send( {'status':true, 'message': 'Salary Range Added Successfully'} );
        })
        .catch( (error)=>{ 
...
controller.editSalaryRangeData = ( req , res ) => {
    ...
    SalaryRangeCI.updateOne( { _id:  dbObjectId( _id ) }, {$set: saveData} )
    .then( (data)=>{ 

        if( data.modifiedCount === 1 ){
            return res.status(200).send( {'status':true, 'message': 'Salary Range Updated Successfully'} );
...
controller.deleteSalaryRangeById = ( req , res ) => {
    ...
    SalaryRangeCI.deleteOne( { _id:  dbObjectId( _id ) } )
    .then( (data)=>{  
        if( data.deletedCount === 1 ){
            return res.status(200).send( {'status':true, 'message': 'Salary Range Deleted Successfully'} );
...
controller.getSalaryRangeById = ( req , res ) => {
    ...
    SalaryRangeCI.find( { _id:  dbObjectId( _id ) } )
    .then( (data)=>{   
        if( data.length > 0 ){ 
...
controller.changeSalaryRangeStatus = ( req , res ) => {
...
controller.getSalaryRangeList = ( req , res ) => {  
    ...
    let searchKeyWord = new RegExp( commonOnly( req.body.keyword ) );
        where['label'] = { $regex: searchKeyWord, $options: 'i' } 
    }
    ...
    if( req.body.hasOwnProperty('filter_keyword') && req.body.filter_keyword !== '' ){
        ...
        where['$or'] = [
            { label: { $regex: searchKeyFilter, $options: 'i' } }, 
            { from: { $regex: searchKeyFilter, $options: 'i' } },
            { to: { $regex: searchKeyFilter, $options: 'i' } }, 
    ...
    SalaryRangeCI.find( where, fetchKeys )
