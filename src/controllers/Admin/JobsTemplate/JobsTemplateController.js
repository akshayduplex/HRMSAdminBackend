const JobTemplatesCI = require('../../../models/JobTemplatesCI.js') ;
...
controller.addJobsTemplate = ( req, res )=>{
    ...
    JobTemplatesCI.findOne( { title: saveData.title, designation_id : dbObjectId( saveData.designation_id ), department_id : dbObjectId( saveData.department_id ) } )
    .then( (ckData)=>{
        if( ckData ){
            return res.status(409).send( {'status':false, 'message': 'Job Template with Same Title Already Added'} );
        }
    
        const instData = new JobTemplatesCI( saveData );
        instData.save()
        .then( (data)=>{
            return res.status(200).send( {'status':true, 'message': 'Job Template Added Successfully'} );
        })
        .catch( (error)=>{ 
...
controller.editJobsTemplate = ( req , res ) => { 
    const errors = validationResult(req);
    ...
    JobTemplatesCI.updateOne( { _id:  dbObjectId( _id ) }, {$set: saveData} )
    .then( (data)=>{ 
...
controller.deleteJobsTemplateById = ( req , res ) => {
    ...
    JobTemplatesCI.deleteOne( { _id:  dbObjectId( _id ) } )
    .then( (data)=>{  
...
controller.getJobsTemplateById = ( req , res ) => {
    ...
    JobTemplatesCI.find( { _id:  dbObjectId( _id ) } )
    .then( (data)=>{   
        if( data.length > 0 ){ 
...
controller.changeJobsTemplateStatus = ( req , res ) => {
...
controller.getJobsTemplateList = ( req , res ) => {  
    ...
    JobTemplatesCI.find( where, fetchKeys )
