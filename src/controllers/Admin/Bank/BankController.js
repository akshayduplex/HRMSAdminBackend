const BankCI = require('../../../models/BankCI.js')
...
controller.addBank = ( req, res )=>{
    ...
    BankCI.findOne( { name: saveData.name } )
    .then( (ckData)=>{
        if( ckData ){
            return res.status(409).send( {'status':false, 'message': 'Bank Already Added'} );
        }
    
        const instData = new BankCI( saveData );
        instData.save()
        .then( (data)=>{
            return res.status(200).send( {'status':true, 'message': 'Bank Added Successfully'} );
        })
        .catch( (error)=>{ 
...
controller.editBank = ( req , res ) => {
    ...
    BankCI.updateOne( { _id:  dbObjectId( _id ) }, {$set: saveData} )
    .then( (data)=>{ 

        if( data.modifiedCount === 1 ){
            return res.status(200).send( {'status':true, 'message': 'Bank Updated Successfully'} );
...
controller.deleteBankById = ( req , res ) => {
    ...
    BankCI.deleteOne( { _id:  dbObjectId( _id ) } )
    .then( (data)=>{  
        if( data.deletedCount === 1 ){
            return res.status(200).send( {'status':true, 'message': 'Bank Deleted Successfully'} );
...
controller.getBankById = ( req , res ) => {
    ...
    BankCI.find( { _id:  dbObjectId( _id ) }, fetchKeys )
    .then( (data)=>{   
        if( data.length > 0 ){ 
...
controller.changeBankStatus = ( req , res ) => {
...
controller.getBankList = ( req , res ) => {  
