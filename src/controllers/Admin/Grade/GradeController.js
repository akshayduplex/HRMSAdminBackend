...
/********* Add New Grade Data **********/
controller.AddGradeData = ( req, res )=>{
    ...
    GradeCI.findOne( { name: saveData.name } )
    .then( (ckData)=>{
        if( ckData ){
            return res.status(409).send( {'status':false, 'message': 'Grade Already Added'} );
        }
    
        const instData = new GradeCI( saveData );
        instData.save()
        .then( (data)=>{
            return res.status(200).send( {'status':true, 'message': 'Grade Added Successfully'} );
        })
        .catch( (error)=>{ 
...
controller.editGradeData = ( req , res ) => {
    ...
    GradeCI.updateOne( { _id:  dbObjectId( _id ) }, {$set: saveData} )
    .then( (data)=>{ 

        if( data.modifiedCount === 1 ){
            return res.status(200).send( {'status':true, 'message': 'Grade Updated Successfully'} );
...
controller.deleteGradeById = ( req , res ) => {
    ...
    GradeCI.deleteOne( { _id:  dbObjectId( _id ) } )
    .then( (data)=>{  
        if( data.deletedCount === 1 ){
            return res.status(200).send( {'status':true, 'message': 'Grade Deleted Successfully'} );
...
controller.getGradeById = ( req , res ) => {
    ...
    GradeCI.find( { _id:  dbObjectId( _id ) } )
    .then( (data)=>{   
        if( data.length > 0 ){ 
...
controller.changeGradeStatus = ( req , res ) => {
...
controller.getGradeList = ( req , res ) => {  
