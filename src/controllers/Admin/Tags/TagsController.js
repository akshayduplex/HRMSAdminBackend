const TagsCl = require('../../../models/TagsCI.js')
...
controller.AddTagData = ( req, res )=>{
    ...
    TagsCl.findOne( { name: saveData.name } )
    .then( (ckData)=>{
        if( ckData ){
            return res.status(409).send( {'status':false, 'message': 'Tag Already Added'} );
        }
    
        const instData = new TagsCl( saveData );
        instData.save()
        .then( (data)=>{
            return res.status(200).send( {'status':true, 'message': 'Tag Added Successfully'} );
        })
        .catch( (error)=>{ 
...
controller.editTagData = ( req , res ) => {
    ...
    TagsCl.updateOne( { _id:  dbObjectId( _id ) }, {$set: saveData} )
    .then( (data)=>{ 

        if( data.modifiedCount === 1 ){
            return res.status(200).send( {'status':true, 'message': 'Tag Updated Successfully'} );
...
controller.deleteTagById = ( req , res ) => {
    ...
    TagsCl.deleteOne( { _id:  dbObjectId( _id ) } )
    .then( (data)=>{  
...
controller.getTagById = ( req , res ) => {
    ...
    TagsCl.find( { _id:  dbObjectId( _id ) } )
    .then( (data)=>{   
        if( data.length > 0 ){ 
...
controller.changeTagStatus = ( req , res ) => {
