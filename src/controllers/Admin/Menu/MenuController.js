const MenuCI = require('../../../models/MenuCI.js')
...
controller.addMenu = ( req, res )=>{
    ...
    MenuCI.findOne( { name: saveData.name } )
    .then( (ckData)=>{
        if( ckData ){
            return res.status(409).send( {'status':false, 'message': 'Menu Already Added'} );
        }
    
        const instData = new MenuCI( saveData );
        instData.save()
        .then( (data)=>{
            return res.status(200).send( {'status':true, 'message': 'Menu Added Successfully'} );
        })
        .catch( (error)=>{ 
...
controller.editMenu = ( req , res ) => {
    ...
    MenuCI.updateOne( { _id:  dbObjectId( _id ) }, {$set: saveData} )
    .then( (data)=>{ 
        
        if( data.modifiedCount === 1 ){
            return res.status(200).send( {'status':true, 'message': 'Menu Updated Successfully'} );
...
controller.deleteMenuById = ( req , res ) => {
    ...
    MenuCI.deleteOne( { _id:  dbObjectId( _id ) } )
    .then( (data)=>{  
        if( data.deletedCount === 1 ){
            return res.status(200).send( {'status':true, 'message': 'Menu Deleted Successfully'} );
...
controller.getMenuById = ( req , res ) => {
    ...
    MenuCI.find( { _id:  dbObjectId( _id ) }, fetchKeys )
    .then( (data)=>{   
        if( data.length > 0 ){ 
...
controller.changeMenuStatus = ( req , res ) => {
