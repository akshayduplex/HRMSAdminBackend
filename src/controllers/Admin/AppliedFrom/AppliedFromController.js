...
/********* Add New Applied From Data **********/
controller.addAppliedFrom = ( req, res )=>{
    ...
    AppliedFromCI.findOne( { name: saveData.name } )
    .then( (ckData)=>{
        if( ckData ){
            return res.status(409).send( {'status':false, 'message': 'Data Already Added'} );
        }
    
        const instData = new AppliedFromCI( saveData );
        instData.save()
        .then( (data)=>{
...
controller.editAppliedFrom = ( req , res ) => {
    ...
    AppliedFromCI.updateOne( { _id:  dbObjectId( _id ) }, {$set: saveData} )
    .then( (data)=>{ 
...
controller.deleteAppliedFromById = ( req , res ) => {
    ...
    AppliedFromCI.deleteOne( { _id:  dbObjectId( _id ) } )
    .then( (data)=>{  
...
controller.getAppliedFromById = ( req , res ) => {
    ...
    AppliedFromCI.find( { _id:  dbObjectId( _id ) } )
    .then( (data)=>{   
        if( data.length > 0 ){ 
...
controller.changeAppliedFromStatus = ( req , res ) => {
...
controller.getAppliedFromList = ( req , res ) => {  
    ...
    AppliedFromCI.find( where, fetchKeys )
