const CurrencyCI = require('../../../models/CurrencyCI.js')
...
controller.addCurrency = ( req, res )=>{
    ...
    CurrencyCI.findOne( { country: saveData.country, code: saveData.code  } )
    .then( (ckData)=>{
        if( ckData ){
            return res.status(409).send( {'status':false, 'message': 'Currency Already Added'} );
        }
    
        const instData = new CurrencyCI( saveData );
        instData.save()
        .then( (data)=>{
            return res.status(200).send( {'status':true, 'message': 'Currency Added Successfully'} );
        })
        .catch( (error)=>{ 
...
controller.editCurrency = ( req , res ) => {
    ...
    CurrencyCI.updateOne( { _id:  dbObjectId( _id ) }, {$set: saveData} )
    .then( (data)=>{ 

        if( data.modifiedCount === 1 ){
            return res.status(200).send( {'status':true, 'message': 'Currency Updated Successfully'} );
...
controller.deleteCurrencyById = ( req , res ) => {
    ...
    CurrencyCI.deleteOne( { _id:  dbObjectId( _id ) } )
    .then( (data)=>{  
        if( data.deletedCount === 1 ){
            return res.status(200).send( {'status':true, 'message': 'Currency Deleted Successfully'} );
...
controller.getCurrencyById = ( req , res ) => {
    ...
    CurrencyCI.find( { _id:  dbObjectId( _id ) }, fetchKeys )
    .then( (data)=>{   
        if( data.length > 0 ){ 
...
controller.changeCurrencyStatus = ( req , res ) => {
...
controller.getCurrencyList = ( req , res ) => {  
    ...
    CurrencyCI.find( where, fetchKeys )
