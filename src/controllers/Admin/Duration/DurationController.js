const DurationCl = require('../../../models/DurationCl.js') ;
...
/********* Add New Duration Data **********/
controller.addDurationData = ( req, res )=>{
    ...
    DurationCl.findOne( { duration: saveData.duration } )
    .then( (ckData)=>{
        if( ckData ){
            return res.status(200).send( {'status':false, 'message': 'Duration Already Added'} );
        }
    
        const instData = new DurationCl( saveData );
        instData.save()
        .then( (data)=>{
            return res.status(200).send( {'status':true, 'message': 'Duration Added Successfully'} );
        })
        .catch( (error)=>{ 
...
controller.editDuration = ( req , res ) => {
    ...
    DurationCl.updateOne( { _id:  dbObjectId( _id ) }, {$set: saveData} )
    .then( (data)=>{ 

        if( data.modifiedCount === 1 ){
            return res.status(200).send( {'status':true, 'message': 'Duration Updated Successfully'} );
...
controller.deleteDuration = ( req , res ) => {
    ...
    DurationCl.deleteOne( { _id:  dbObjectId( _id ) } )
    .then( (data)=>{  
        if( data.deletedCount === 1 ){
            return res.status(200).send( {'status':true, 'message': 'Duration Deleted Successfully'} );
...
controller.getDurationById = ( req , res ) => {
    ...
    DurationCl.find( { _id:  dbObjectId( _id ) } )
    .then( (data)=>{   
        if( data.length > 0 ){ 
...
controller.changeDurationStatus = ( req , res ) => {
...
controller.getDurationList = ( req , res ) => {  
    ...
    let searchKeyWord = new RegExp( lettersOnly( req.body.keyword ) );
        where['duration'] = { $regex: searchKeyWord, $options: 'i' } 
    } 
    ...
    if( req.body.hasOwnProperty('filter_keyword') && req.body.filter_keyword !== '' ){
        ...
        where['$or'] = [
            { duration: { $regex: searchKeyFilter, $options: 'i' } }, 
    ...
    DurationCl.find( where, fetchKeys )
