const DivisionCI = require('../../../models/DivisionCI.js')
...
controller.addDivision = ( req, res )=>{
    ...
    DivisionCI.findOne( { name: saveData.name } )
    .then( (ckData)=>{
        if( ckData ){
            return res.status(409).send( {'status':false, 'message': 'Division Already Added'} );
        }
    
        const instData = new DivisionCI( saveData );
        instData.save()
        .then( (data)=>{
            return res.status(200).send( {'status':true, 'message': 'Division Added Successfully'} );
        })
        .catch( (error)=>{ 
...
controller.editDivisionData = ( req , res ) => {
    ...
    DivisionCI.updateOne( { _id:  dbObjectId( _id ) }, {$set: saveData} )
    .then( (data)=>{ 

        if( data.modifiedCount === 1 ){
            return res.status(200).send( {'status':true, 'message': 'Division Updated Successfully'} );
...
controller.deleteDivisionById = ( req , res ) => {
    ...
    DivisionCI.deleteOne( { _id:  dbObjectId( _id ) } )
    .then( (data)=>{  
        if( data.deletedCount === 1 ){
            return res.status(200).send( {'status':true, 'message': 'Division Deleted Successfully'} );
...
controller.getDivisionById = ( req , res ) => {
    ...
    DivisionCI.find( { _id:  dbObjectId( _id ) }, fetchKeys )
    .then( (data)=>{   
        if( data.length > 0 ){ 
...
controller.changeDivisionStatus = ( req , res ) => {
...
controller.getDivisionList = ( req , res ) => {  
    ...
    DivisionCI.find( where, fetchKeys )
