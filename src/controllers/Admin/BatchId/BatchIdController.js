const BatchIdCI = require('../../../models/BatchIdCI.js')
...
/********* Add New BatchID Data **********/
controller.addBatchID = ( req, res )=>{
    ...
    BatchIdCI.findOne( { batch_id: saveData.batch_id } )
    .then( (ckData)=>{
        if( ckData ){
            return res.status(409).send( {'status':false, 'message': 'Batch ID Already Added'} );
        }
    
        const instData = new BatchIdCI( saveData );
        instData.save()
        .then( (data)=>{
            return res.status(200).send( {'status':true, 'message': 'Batch ID Added Successfully'} );
        })
        .catch( (error)=>{
...
controller.editBatchID = ( req , res ) => {
    ...
    BatchIdCI.updateOne( { _id:  dbObjectId( _id ) }, {$set: saveData} )
    .then( (data)=>{ 

        if( data.modifiedCount === 1 ){
            return res.status(200).send( {'status':true, 'message': 'Batch ID Updated Successfully'} );
...
controller.deleteBatchIDById = ( req , res ) => {
    ...
    BatchIdCI.deleteOne( { _id:  dbObjectId( _id ) } )
    .then( (data)=>{  
        if( data.deletedCount === 1 ){
            return res.status(200).send( {'status':true, 'message': 'Batch ID Deleted Successfully'} );
...
controller.getBatchIDById = ( req , res ) => {
    ...
    BatchIdCI.find( { _id:  dbObjectId( _id ) }, fetchKeys )
    .then( (data)=>{   
        if( data.length > 0 ){ 
...
controller.changeBatchIDStatus = ( req , res ) => {
...
controller.getBatchIDList = ( req , res ) => {  
    ...
    let searchKeyWord = new RegExp( lettersOnly( req.body.keyword ) );
        //where['batch_id'] = { $regex: searchKeyWord, $options: 'i' }
        where['batch_id'] = req.body.keyword ;
    } 
    ...
    BatchIdCI.find( where, fetchKeys )
