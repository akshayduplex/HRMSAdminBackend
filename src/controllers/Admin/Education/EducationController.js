const EducationCl = require('../../../models/EducationCI.js');
...
controller.AddEducationData = ( req, res )=>{
    ...
    EducationCl.findOne( { name: saveData.name } )
    .then( (ckData)=>{
        if( ckData ){
            return res.status(409).send( {'status':false, 'message': 'Education Already Added'} );
        }
    
        const instData = new EducationCl( saveData );
        instData.save()
        .then( (data)=>{
            return res.status(200).send( {'status':true, 'message': 'Education Added Successfully'} );
        })
        .catch( (error)=>{ 
...
controller.editEducationData = ( req , res ) => {
    ...
    EducationCl.updateOne( { _id:  dbObjectId( _id ) }, {$set: saveData} )
    .then( (data)=>{ 

        if( data.modifiedCount === 1 ){
            return res.status(200).send( {'status':true, 'message': 'Education Updated Successfully'} );
...
controller.deleteEducationById = ( req , res ) => {
    ...
    EducationCl.deleteOne( { _id:  dbObjectId( _id ) } )
    .then( (data)=>{  
        if( data.deletedCount === 1 ){
            return res.status(200).send( {'status':true, 'message': 'Education Deleted Successfully'} );
...
controller.getEducationById = ( req , res ) => {
    ...
    EducationCl.find( { _id:  dbObjectId( _id ) } )
    .then( (data)=>{
        if( data.length > 0 ){
...
controller.changeEducationStatus = ( req , res ) => {
...
controller.getEducationList = ( req , res ) => {  
    ...
    EducationCl.find( where, fetchKeys )
