...
controller.addState = ( req, res )=>{
    ...
    StateCI.findOne( { name: saveData.name } )
    .then( (ckData)=>{
        if( ckData ){
            return res.status(409).send( {'status':false, 'message': 'State Already Added'} );
        }
    
        const instData = new StateCI( saveData );
        instData.save()
        .then( (data)=>{
...
controller.editState = ( req , res ) => {
    ...
    StateCI.updateOne( { _id:  dbObjectId( _id ) }, {$set: saveData} )
    .then( (data)=>{ 
...
controller.deleteStateById = ( req , res ) => {
    ...
    StateCI.deleteOne( { _id:  dbObjectId( _id ) } )
    .then( (data)=>{  
...
controller.getStateById = ( req , res ) => {
    ...
    StateCI.find( { _id:  dbObjectId( _id ) }, fetchKeys )
    .then( (data)=>{   
        if( data.length > 0 ){ 
...
controller.changeStateStatus = ( req , res ) => {
...
controller.getStateList = ( req , res ) => {  
