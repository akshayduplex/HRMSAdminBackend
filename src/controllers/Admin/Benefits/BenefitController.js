const BenefitsCl = require('../../../models/BenefitsCI.js')
...
controller.AddBenefitsData = ( req, res )=>{
    ...
    BenefitsCl.findOne( { name: saveData.name } )
    .then( (ckData)=>{
        if( ckData ){
            return res.status(409).send( {'status':false, 'message': 'Benefit Already Added'} );
        }
    
        const instData = new BenefitsCl( saveData );
        instData.save()
        .then( (data)=>{
            return res.status(200).send( {'status':true, 'message': 'Benefit Added Successfully'} );
        })
        .catch( (error)=>{ 
...
controller.editBenefitsData = ( req , res ) => {
    ...
    BenefitsCl.updateOne( { _id:  dbObjectId( _id ) }, {$set: saveData} )
    .then( (data)=>{ 

        if( data.modifiedCount === 1 ){
            return res.status(200).send( {'status':true, 'message': 'Benefit Updated Successfully'} );
...
controller.deleteBenefitsById = ( req , res ) => {
    ...
    BenefitsCl.deleteOne( { _id:  dbObjectId( _id ) } )
    .then( (data)=>{  
        if( data.deletedCount === 1 ){
            return res.status(200).send( {'status':true, 'message': 'Benefit Deleted Successfully'} );
...
controller.getBenefitsById = ( req , res ) => {
    ...
    BenefitsCl.find( { _id:  dbObjectId( _id ) } )
    .then( (data)=>{   
        if( data.length > 0 ){ 
...
controller.changeBenefitsStatus = ( req , res ) => {
...
controller.getBenefitsList = ( req , res ) => {  
    ...
    BenefitsCl.find( where, fetchKeys )
