...
controller.AddDepartmentData = ( req, res )=>{
    ...
    DepartmentCl.findOne( { name: saveData.name } )
    .then( (ckData)=>{
        if( ckData ){
            return res.status(409).send( {'status':false, 'message': 'Department Already Added'} );
        }
    
        const instData = new DepartmentCl( saveData );
        instData.save()
        .then( (data)=>{
...
controller.editDepartmentData = ( req , res ) => {
    ...
    DepartmentCl.updateOne( { _id:  dbObjectId( _id ) }, {$set: saveData} )
    .then( (data)=>{ 
...
controller.deleteDepartmentById = ( req , res ) => {
    ...
    DepartmentCl.deleteOne( { _id:  dbObjectId( _id ) } )
    .then( (data)=>{  
...
controller.getDepartmentById = ( req , res ) => {
    ...
    DepartmentCl.find( { _id:  dbObjectId( _id ) } )
    .then( (data)=>{   
        if( data.length > 0 ){ 
...
controller.changeDepartmentStatus = ( req , res ) => {
...
controller.getDepartmentList = ( req , res ) => {  
    ...
    DepartmentCl.find( where, fetchKeys )
