const OccupationCI = require('../../../models/OccupationCI.js')
const { dbObjectId } = require('../../../models/dbObject.js');
const dotenv = require("dotenv");
dotenv.config({path:'../src/config.env'});
const { dbDateFormat, updateDatesInArray,replaceNullUndefined , commonOnly } = require('../../../middlewares/myFilters.js');
...
controller.addOccupation = ( req, res )=>{
    ...
    OccupationCI.findOne( { name: saveData.name } )
    .then( (ckData)=>{
        if( ckData ){
            return res.status(409).send( {'status':false, 'message': 'Occupation Already Added'} );
        }
    
        const instData = new OccupationCI( saveData );
        instData.save()
        .then( (data)=>{
...
controller.editOccupation = ( req , res ) => {
    ...
    OccupationCI.updateOne( { _id:  dbObjectId( _id ) }, {$set: saveData} )
    .then( (data)=>{ 

        if( data.modifiedCount === 1 ){
            return res.status(200).send( {'status':true, 'message': 'Occupation Updated Successfully'} );
...
controller.deleteOccupationById = ( req , res ) => {
    ...
    OccupationCI.deleteOne( { _id:  dbObjectId( _id ) } )
    .then( (data)=>{  
        if( data.deletedCount === 1 ){
            return res.status(200).send( {'status':true, 'message': 'Occupation Deleted Successfully'} );
...
controller.getOccupationById = ( req , res ) => {
    ...
    OccupationCI.find( { _id:  dbObjectId( _id ) } )
    .then( (data)=>{   
        if( data.length > 0 ){ 
...
controller.changeOccupationStatus = ( req , res ) => {
...
controller.getOccupationList = ( req , res ) => {  
    ...
    OccupationCI.find( where, fetchKeys )
