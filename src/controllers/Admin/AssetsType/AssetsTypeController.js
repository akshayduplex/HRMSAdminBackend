const AssetsTypeCI = require('../../../models/AssetsTypeCI.js')
...
/********* Add New Assets Type Data **********/
controller.addAssetType = ( req, res )=>{
    ...
    AssetsTypeCI.findOne( { name: saveData.name } )
    .then( (ckData)=>{
        if( ckData ){
            return res.status(409).send( {'status':false, 'message': 'Asset Type Already Added'} );
        }
    
        const instData = new AssetsTypeCI   ( saveData );
        instData.save()
        .then( (data)=>{
            return res.status(200).send( {'status':true, 'message': 'Asset Type Added Successfully'} );
        })
        .catch( (error)=>{ 
...
controller.editAssetType = ( req , res ) => {
    ...
    AssetsTypeCI.updateOne( { _id:  dbObjectId( _id ) }, {$set: saveData} )
    .then( (data)=>{ 

        if( data.modifiedCount === 1 ){
            return res.status(200).send( {'status':true, 'message': 'Asset Type Updated Successfully'} );
...
controller.deleteAssetTypeById = ( req , res ) => {
    ...
    AssetsTypeCI.deleteOne( { _id:  dbObjectId( _id ) } )
    .then( (data)=>{  
        if( data.deletedCount === 1 ){
            return res.status(200).send( {'status':true, 'message': 'Asset Type Deleted Successfully'} );
...
controller.getAssetTypeById = ( req , res ) => {
    ...
    AssetsTypeCI.find( { _id:  dbObjectId( _id ) }, fetchKeys )
    .then( (data)=>{   
        if( data.length > 0 ){ 
...
controller.changeAssetTypeStatus = ( req , res ) => {
...
controller.getAssetTypeList = ( req , res ) => {  
    ...
    AssetsTypeCI.find( where, fetchKeys )
