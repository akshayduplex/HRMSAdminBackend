const DispensaryCI = require('../../../models/DispensaryCI.js')
...
/********* Add New Dispensary Data **********/
controller.addDispensary = ( req, res )=>{
    ...
    DispensaryCI.findOne( { name: saveData.name } )
    .then( (ckData)=>{
        if( ckData ){
            return res.status(409).send( {'status':false, 'message': 'Dispensary Already Added'} );
        }
    
        const instData = new DispensaryCI( saveData );
        instData.save()
        .then( (data)=>{
            return res.status(200).send( {'status':true, 'message': 'Dispensary Added Successfully'} );
        })
        .catch( (error)=>{ 
...
controller.editDispensary = ( req , res ) => {
    ...
    DispensaryCI.updateOne( { _id:  dbObjectId( _id ) }, {$set: saveData} )
    .then( (data)=>{ 

        if( data.modifiedCount === 1 ){
            return res.status(200).send( {'status':true, 'message': 'Dispensary Updated Successfully'} );
...
controller.deleteDispensaryById = ( req , res ) => {
    ...
    DispensaryCI.deleteOne( { _id:  dbObjectId( _id ) } )
    .then( (data)=>{  
        if( data.deletedCount === 1 ){
            return res.status(200).send( {'status':true, 'message': 'Dispensary Deleted Successfully'} );
...
controller.getDispensaryById = ( req , res ) => {
    ...
    DispensaryCI.find( { _id:  dbObjectId( _id ) }, fetchKeys )
    .then( (data)=>{   
        if( data.length > 0 ){ 
...
controller.changeDispensaryStatus = ( req , res ) => {
...
controller.getDispensaryList = ( req , res ) => {  
    ...
    DispensaryCI.find( where, fetchKeys )
